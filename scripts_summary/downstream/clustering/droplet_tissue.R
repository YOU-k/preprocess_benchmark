...
droplet_tissue_hvged <- readRDS(file.path(read.path,"droplet_tissue_hvged.rds"))


droplet_tissue_hvged %>% filter(!data=="Cell Ranger") -> droplet_tissue_hvged
dt_cellranger <- readRDS(file.path(read.path,"droplet_tissue_hvged_cellranger.rds"))
bind_rows(droplet_tissue_hvged,dt_cellranger) %>% filter(!norm_method=="sctransform") %>% 
  arrange(design,data,norm_method,hivar_method) -> droplet_tissue_hvged
...
droplet_tissue_hvged %>% apply_methods(cluster_method) ->result2
saveRDS(result2,file.path(write.path,"droplet_tissue.rds"))
###evaluation
library(mclust)
...
saveRDS(result3,file.path(write.path,"droplet_tissue_evals.rds"))
