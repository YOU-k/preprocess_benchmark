...
linnorm_norm = function(sce){
  if(table(apply(counts(sce), 1, function(x) sum(x>0)/length(x)) >0.4)[2] <200){
...
saveRDS(result1,file.path(write.path,"plate_singlecell.rds"))
...
saveRDS(result2,file.path(write.path,"eval/plate_singlecell_eval.rds"))
