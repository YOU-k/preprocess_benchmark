...
linnorm_norm = function(sce){
  if(table(apply(counts(sce), 1, function(x) sum(x>0)/length(x)) >0.5)[2] <200){
      logcounts(sce) = Linnorm(counts(sce),minNonZeroPortion = 0.3)
  }else if(table(apply(counts(sce), 1, function(x) sum(x>0)/length(x)) >0.75)[2] <200){
      logcounts(sce) = Linnorm(counts(sce),minNonZeroPortion = 0.5)
  }else{
...
sctransform_norm = function(sce){
  ...
  norm_seuratz<- SCTransform(object = seurat, verbose = FALSE,variable.features.n = 1000)
...
saveRDS(result1,file.path(write.path,"plate_mixture.rds"))
...
saveRDS(result2,file.path(write.path,"eval/plate_mixture_eval.rds"))
