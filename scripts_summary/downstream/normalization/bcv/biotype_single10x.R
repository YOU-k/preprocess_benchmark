...
drop_raw %>% filter(data=="cellranger") %>% arrange(design,data) %>% apply_methods(trend) ->  drop_raw_trend_cellranger

bind_rows(drop_raw_trend %>% filter(!data=="cellranger"),drop_raw_trend_cellranger) %>% filter(!is.task_error(result))-> drop_raw_trend
...
saveRDS(drop_raw_trend_spread,file.path(write.path,"biotypes/drop_raw_trend_singlecell.rds"))
