...
remove_doublet<- function(sce){
  sce$doublet[is.na(sce$doublet)] <- "SNG"
  return(sce[,!sce$doublet=="DBL"])
}
...
saveRDS(result1,file.path(write.path,"sc5clv3_norm.rds"))
...
saveRDS(result2,file.path(write.path,"eval/sc5clv3_eval.rds"))
