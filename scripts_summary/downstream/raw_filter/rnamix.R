...
plate_info <- read.csv("/stornext/General/data/user_managed/grpu_mritchie_1/Yue/preprocess/infos/NN84_mRNA_annotation_correct.csv")
#M19 is empty, delete plate_info[307,]
...
library(scPipe)
sc1_p <- "/stornext/General/data/user_managed/grpu_mritchie_1/Yue/preprocess/scpipe/nn84-2/"

sc1 <- create_sce_by_dir(sc1_p, organism = "hsapiens_gene_ensembl", gene_id_type="ensembl_gene_id")
colData(sc1) = cbind(colData(sc1), DataFrame(plate_info[match(colnames(sc1),as.character(plate_info$cell_name)),]))
...
#plotfc(scp_sc1_qc,"rnamix/scpipe_rmout_fc")
...
table(sc1_f)
sc1$keep <-sc1_f
sc1$group <- paste(sc1$H1975_prop,sc1$H2228_prop,sc1$HCC827_prop)

saveRDS(gene_filter(sc1[,sc1$keep]),file.path(write.path,"rnamix_scpipe.rds"))

#plotfc(sc1,"rnamix/scpipe_scater_fc")
...
zumis_path <- "/stornext/General/data/user_managed/grpu_mritchie_1/Yue/preprocess/zumis2/nn84-2/zUMIs_output/expression"
...
zumis_sc1 <- SingleCellExperiment(
  ...
  colData = plate_info[match(colnames(count_sc1),as.character(plate_info$cell_name)),]
)
...
#plotfc(zumis_sc1,"rnamix/zumis_scater_fc")
...
zumis_sc2 <- SingleCellExperiment(
  ...
  colData = plate_info[match(colnames(count_sc2),as.character(plate_info$cell_name)),]
)
colnames(zumis_sc2) <- colnames(count_sc2)

zumis_sc2 <- zumis_sc2[,!colnames(zumis_sc2) %in% notwanted]
...
zumis_sc1$group <- paste(zumis_sc1$H1975_prop,zumis_sc1$H2228_prop,zumis_sc1$HCC827_prop)

saveRDS(gene_filter(zumis_sc1[,zumis_sc1$keep]),file.path(write.path,"rnamix_zumis.rds"))

#celseq2
cs_path <-"/stornext/General/data/user_managed/grpu_mritchie_1/Yue/preprocess/celseq2/nn84-2/result/expr/nn84-2/"
...
celseq_sc1 <- SingleCellExperiment(
  ...
  colData = plate_info[match(colnames(cs_mat),as.character(plate_info$cell_name)),]
)
...
celseq_sc1$group <- paste(celseq_sc1$H1975_prop,celseq_sc1$H2228_prop,celseq_sc1$HCC827_prop)
#plotfc(celseq_sc1,"rnamix/celseq_scater_fc")
...
saveRDS(gene_filter(celseq_sc1[,celseq_sc1$keep]),file.path(write.path,"rnamix_celseq.rds"))



###scruff
library(scruff)
scruff.path <- "/stornext/General/data/user_managed/grpu_mritchie_1/Yue/preprocess/scruff/nn84-2/Count/"
load(file.path(scruff.path,"20200819_162813_countUMI_sce.rda"))
...
colData(scruff_sc1) = cbind(colData(scruff_sc1),plate_info[match(scruff_sc1$samplename,as.character(plate_info$cell_name)),])
...
scruff_sc1$group <- paste(scruff_sc1$H1975_prop,scruff_sc1$H2228_prop,scruff_sc1$HCC827_prop)
...
#plotfc(scruff_sc1,"rnamix/scruff_scater_fc")
...
saveRDS(gene_filter(scruff_sc1[,scruff_sc1$keep]),file.path(write.path,"rnamix_scruff.rds"))

#kallisto 

library(Matrix)
kb_path <-"/stornext/General/data/user_managed/grpu_mritchie_1/Yue/preprocess/kallisto_bus/nn84-2/genecount/"
...
kb_sc1 <- SingleCellExperiment(
  ...
  colData = plate_info[match(colnames(kb_mat),as.character(plate_info$cell_name)),]
)
...
kb_sc1$keep <-kb_f
#plotfc(kb_sc1,"rnamix/kb_scater_fc")
...
kb_sc1$group <- paste(kb_sc1$H1975_prop,kb_sc1$H2228_prop,kb_sc1$HCC827_prop)
saveRDS(gene_filter(kb_sc1[,kb_sc1$keep]),file.path(write.path,"rnamix_kb.rds"))


###all together

#zero
bind_rows(zero.d.celseq2,zero.d.scpipe,zero.d.scruff,zero.d.zumis,zero.d.kb) ->zero.d
saveRDS(zero.d,"SCE/raw/zeros/rnamix_zero.rds")

pdf("results/raw/rnamix/zero_all.pdf",width = 5,height = 4)
...
pdf("results/raw/rnamix/zero.pdf",width = 8,height = 4)
...
pdf("results/raw/rnamix/total_counts_density.pdf",width = 5,height = 4)
...
saveRDS(gene_biotype,"SCE/raw/biotypes/rnamix_gene_biotype.rds")
...
pdf("results/raw/rnamix/biotype_count.pdf",width =6, height = 4)
...
pdf("results/raw/rnamix/upsetr.pdf",width = 5,height = 5)
...
pdf("results/raw/rnamix/total_counts_boxplot.pdf",width = 5,height = 3.5)
...
apply_methods(datasets,libm) -> lib
pdf("results/raw/rnamix/genes_boxplot.pdf",width = 5,height = 3.5)
...
pdf("results/raw/rnamix/total_counts_overlap.pdf",width = 5,height = 3.5)
...
pdf("results/raw/rnamix/genes_overlap.pdf",width = 5,height = 3.5)
...
saveRDS(dgenes,"SCE/raw/overlap/rnamix_dgenes.rds")
saveRDS(umi_counts,"SCE/raw/overlap/rnamix_umi_counts.rds")
...
pdf("results/raw/rnamix/correlation.pdf",width = 5,height = 3.5)
...
saveRDS(cors,"SCE/raw/cor/rnamix_cors.rds")

#filter 

bind_rows(filter.celseq,filter.kb,filter.scpipe,filter.scpipe.rm,filter.scruff,filter.zumis) ->filter.d
saveRDS(filter.d,"SCE/raw/filter/rnamix_filter.rds")

library(ggsci)
#mito

pdf("results/raw/rnamix/mito_filter.pdf")
...
#total
pdf("results/raw/rnamix/total_filter.pdf",width = 6,height = 4)
...
#detected
pdf("results/raw/rnamix/detected_filter.pdf",width = 6,height = 4)
