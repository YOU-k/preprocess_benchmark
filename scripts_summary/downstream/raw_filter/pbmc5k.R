...
barcode_info <- read.csv("/stornext/HPCScratch/home/you.y/preprocess_update/pbmc_data/5k_pbmc/label_file.csv",header = TRUE)
sapply(strsplit(as.character(barcode_info$barcode),"-"),function(x) x[1]) -> barcode_info$barcode

barcode_info %>% dplyr::select(barcode, label) -> barcode_info
barcode_info$SNG.1ST <- barcode_info$label
...
gene_filter <- function(sce){
  keep1 = (apply(as.matrix(counts(sce)), 1, function(x) mean(x[x>0])) > 1)  # average count larger than 1
  keep2 = (rowSums(as.matrix(counts(sce)>0)) > 10)  
  table(keep1&keep2)
  sce = sce[(keep1 & keep2), ]
  return(sce)
}
...
doublet_bc <- readRDS("/stornext/HPCScratch/home/you.y/preprocess_update/pbmc_data/5k_pbmc/cd_rm.rds")
sapply(strsplit(as.character(doublet_bc),"-"),function(x) x[1]) -> doublet_bc
table(colnames(sc11) %in% doublet_bc)


##scpipe 
library(DropletUtils)
library(scPipe)
sc1_p <- "/stornext/HPCScratch/home/you.y/preprocess_update/raw_results/scpipe/pbmc5k/"
sc1 <- create_sce_by_dir(sc1_p, organism = "hsapiens_gene_ensembl", gene_id_type="ensembl_gene_id")
sc1_barcode <- read.csv("/stornext/HPCScratch/home/you.y/preprocess_update/raw_results/scpipe/pbmc5k/barcode_anno.csv",header = TRUE)
...
saveRDS(sc11,file.path(write.path,"pbm5k_scpipe.rds"))

#zumis

library(SingleCellExperiment)
zumis_path <- "/stornext/HPCScratch/home/you.y/preprocess_update/raw_results/zumis/pbmc5k_v2/zUMIs_output/expression/"
zumis_sc1_data <- readRDS(file.path(zumis_path,"pbmc5k.dgecounts.rds"))
...
saveRDS(zumis_sc11,file.path(write.path,"pbmc5k_zumis.rds"))

#kallisto 

library(Matrix)
kb_path <-"/stornext/HPCScratch/home/you.y/preprocess_update/raw_results/kb/pbmc5k/genecount/"
...
saveRDS(kb_sc11,file.path(write.path,"pbmc5k_kb.rds"))
...
library(fishpond)
library(tximport)
sal_ale_path<-"/stornext/General/data/user_managed/grpu_mritchie_1/Yue/preprocess_update/raw_results/SA/pbmc5k/output/alevin/"
...
alevin_sc <- SingleCellExperiment(
  assays = list(counts = as.matrix(alevin$counts))
)
...
saveRDS(alevin_sc1,file.path(write.path,"pbmc5k_sa.rds"))


#splici
source("/stornext/General/data/user_managed/grpu_mritchie_1/Yue/preprocess_update/raw_results/splici/load_fry.R")
splici_path <-"/stornext/General/data/user_managed/grpu_mritchie_1/Yue/preprocess_update/tools/salmon/quants/pbmc5k/quant/"
af_all = load_fry(splici_path)
af_empty <- emptyDrops(counts(af_all))
saveRDS(af_empty,file.path(write.path,"pbmc5k_splici_empty.rds"))
af  <- af_all[,which(af_empty$FDR<=0.001)]
#af  <- af[,!is.na(af_empty$FDR)]
#af <- af_all[,defaultDrops(counts(af_all),expected = 5000)]
...
saveRDS(af1,file.path(write.path,"pbmc5k_splici.rds"))


#dropseqpipe

dropseq_path <- "/stornext/General/data/user_managed/grpu_mritchie_1/Yue/preprocess_update/raw_results/dsp/pbmc5k/results/summary/umi/"
...
saveRDS(drop_sc11,file.path(write.path,"pbmc5k_drop.rds"))


#cellranger
cellr_path <-"/stornext/HPCScratch/home/you.y/preprocess_update/raw_results/cellranger/pbmc5k/5k_pbmc_v3/outs/raw_feature_bc_matrix/"
...
saveRDS(cellrangr_sc1,file.path(write.path,"pbmc5k_cellranger.rds"))
