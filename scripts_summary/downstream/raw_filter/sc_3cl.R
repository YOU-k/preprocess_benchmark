...
barcode_info <- read.table("/stornext/General/data/user_managed/grpu_mritchie_1/Yue/preprocess/sc_mixology/misc/demuxlet_output/sc_10X.best",header = TRUE)
...
library(scPipe)
sc1_p <- "/stornext/General/data/user_managed/grpu_mritchie_1/Yue/preprocess/scpipe/sc_3cl2/"
sc1 <- create_sce_by_dir(sc1_p, organism = "hsapiens_gene_ensembl", gene_id_type="ensembl_gene_id")
sc1_barcode <- read.csv("/stornext/General/data/user_managed/grpu_mritchie_1/Yue/preprocess/scpipe/sc_3cl2/barcode_anno.csv",header = TRUE)
colData(sc1) = cbind(colData(sc1), barcode=sc1_barcode$barcode_sequence)

colnames(sc1) <- sc1$barcode

sc_empty <- readRDS(file.path(empty.path,"scpipe_3cl_emptied.rds"))
...
sc2_p <- "/stornext/General/data/user_managed/grpu_mritchie_1/Yue/preprocess/scpipe/sc_3cl/"
sc2 <- create_sce_by_dir(sc2_p, organism = "hsapiens_gene_ensembl", gene_id_type="ensembl_gene_id")
sc2_barcode <- read.csv("/stornext/General/data/user_managed/grpu_mritchie_1/Yue/preprocess/scpipe/sc_3cl/barcode_anno.csv",header = TRUE)
...
plotfc(scp_sc1_qc,"sc_3cl/fc/scpipe_rmout_fc")
...
plotfc(sc1,"sc_3cl/fc/scpipe_scater_fc")
...
saveRDS(gene_filter(sc1[,sc1$keep]),file.path(write.path,"sc3cl_scpipe.rds"))

#zumis

library(SingleCellExperiment)
library(DropletUtils)
zumis_path <- "/stornext/General/data/user_managed/grpu_mritchie_1/Yue/preprocess/zumis2/sc_3cl/zUMIs_output/expression"
zumis_sc1_data <- readRDS(file.path(zumis_path,"sc_3cl.dgecounts.rds"))
...
#too many cells
#zumis_empty <- readRDS(file.path(empty.path,"zumis_3cl_emptied.rds"))
defaultDrops(zumis_sc1_data$umicount$inex$all) -> c.keep
zumis_sc1 <- zumis_sc1[,c.keep]
...
plotfc(zumis_sc1,"sc_3cl/fc/zumis_scater_fc")


#automatic
zumis_path2 <- "/stornext/General/data/user_managed/grpu_mritchie_1/Yue/preprocess/zumis/sc_3cl/zUMIs_output/expression"
zumis_sc2_data <- readRDS(file.path(zumis_path2,"sc_3cl.dgecounts.rds"))
...
plotfc(zumis_sc1,"sc_3cl/fc/zumis_auto_scater_fc")
...
saveRDS(gene_filter(zumis_sc1[,zumis_sc1$keep]),file.path(write.path,"sc3cl_zumis.rds"))
...
#
#zumis_empty2 <- readRDS(file.path(empty.path,"zumis_3cl_emptied2.rds"))
#zumis_sc3 <- zumis_sc3[,which(zumis_empty2$FDR<=0.001)]

defaultDrops(zumis_sc1_data$umicount$exon$all) -> c.keep
zumis_sc3 <- zumis_sc3[,c.keep]
...
kb_path <-"/stornext/General/data/user_managed/grpu_mritchie_1/Yue/preprocess/kallisto_bus/sc_3cl/genecount/"
...
defaultDrops(kb_mat) -> c.keep

kb_sc1<- SingleCellExperiment(
  assays = list(counts = as.matrix(kb_mat[,which(c.keep)]))
)
...
kb_sc1$keep <-kb_f
kb_sc1 <- kb_sc1[rowSums(counts(kb_sc1))>0,]
plotfc(kb_sc1,"sc_3cl/fc/kb_scater_fc")
...
saveRDS(gene_filter(kb_sc1[,kb_sc1$keep]),file.path(write.path,"sc3cl_kb.rds"))
...
library(fishpond)
alevin_empty <- readRDS(file.path(empty.path,"alevin_3cl.rds"))
library(tximport)
sal_ale_path<-"/stornext/General/data/user_managed/grpu_mritchie_1/Yue/preprocess/sal_ale/sc_3cl/alevin_output/alevin/"
...
plotfc(alevin_sc,"sc_3cl/fc/alevin_scater_fc")
...
saveRDS(gene_filter(alevin_sc[,alevin_sc$keep]),file.path(write.path,"sc3cl_alevin.rds"))

#dropseqpipe
#######
dropseq_path <- "/stornext/General/data/user_managed/grpu_mritchie_1/Yue/preprocess/dsp/sc_3cl/results/summary/umi"
...
drop_empty <- readRDS(file.path(empty.path,"dropseqpipe_3cl_emptied.rds"))
...
plotfc(drop_sc1,"sc_3cl/fc/drop_scater_fc")
...
saveRDS(gene_filter(drop_sc1[,drop_sc1$keep]),file.path(write.path,"sc3cl_drop.rds"))



#cellranger
cellranger_empty <- readRDS(file.path(empty.path,"cellranger_3cl_emptied.rds"))
cellr_path <-"/stornext/General/data/user_managed/grpu_mritchie_1/Yue/preprocess/cellranger/sc_3cl/Undetermined/outs/raw_feature_bc_matrix/"
...
plotfc(cellrangr_sc,"sc_3cl/fc/cellranger_scater_fc")
...
saveRDS(gene_filter(cellrangr_sc[,cellrangr_sc$keep]),file.path(write.path,"sc3cl_cellranger.rds"))

#optimus
optimus_path <- "/stornext/General/data/user_managed/grpu_mritchie_1/Yue/preprocess/optimus/sc3cl/"
...
plotfc(optimus_sc,"sc_3cl/fc/optimus_scater_fc")
...
saveRDS(gene_filter(optimus_sc[,optimus_sc$keep]),file.path(write.path,"sc3cl_optimus.rds"))
