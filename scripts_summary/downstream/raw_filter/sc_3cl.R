...
barcode_info$barcode <- barcode_info$BARCODE
...
library(scPipe)
sc1_p <- "/stornext/HPCScratch/home/you.y/preprocess_update/raw_results/scpipe/sc_3cl/"
sc1 <- create_sce_by_dir(sc1_p, organism = "hsapiens_gene_ensembl", gene_id_type="ensembl_gene_id")
sc1_barcode <- read.csv("/stornext/HPCScratch/home/you.y/preprocess_update/raw_results/scpipe/sc_3cl/barcode_anno.csv",header = TRUE)
...
saveRDS(sc_empty,file.path(write.path,"sc3cl_scpipe_empty.rds"))
...
saveRDS(sc11,file.path(write.path,"sc3cl_scpipe.rds"))

#zumis

library(SingleCellExperiment)
zumis_path <- "/stornext/HPCScratch/home/you.y/preprocess_update/raw_results/zumis/sc_3cl/zUMIs_output/expression/"
...
saveRDS(zumis_sc11,file.path(write.path,"sc3cl_zumis_exon.rds"))

#kallisto 

library(Matrix)
kb_path <-"/stornext/HPCScratch/home/you.y/preprocess_update/raw_results/kb/sc_3cl/genecount/"
...
saveRDS(kb_empty,file.path(write.path,"sc3cl_kb_empty.rds"))

kb_sc1<- SingleCellExperiment(
  assays = list(counts = as.matrix(kb_mat[,defaultDrops(kb_mat)]))
)
...
saveRDS(kb_sc11,file.path(write.path,"sc3cl_kb.rds"))
...
library(fishpond)
library(tximport)
sal_ale_path<-"/stornext/General/data/user_managed/grpu_mritchie_1/Yue/preprocess_update/raw_results/SA/sc_3cl/output/alevin/"

alevin <- tximport(file.path(sal_ale_path,"quants_mat.gz"),type="alevin")
alevin_empty <- emptyDrops(alevin$counts,lower = 100)
saveRDS(alevin_empty,file.path(write.path,"sc3cl_sa_empty.rds"))
...
alevin_sc$doublet <- barcode_info$doublet[match(colnames(alevin_sc), barcode_info$barcode)]
...
saveRDS(alevin_sc1,file.path(write.path,"sc3cl_sa.rds"))


#splici
source("/stornext/General/data/user_managed/grpu_mritchie_1/Yue/preprocess_update/raw_results/splici/load_fry.R")
splici_path <-"/stornext/General/data/user_managed/grpu_mritchie_1/Yue/preprocess_update/tools/salmon/quants/sc_3cl/quant/"
...
af$doublet <- barcode_info$doublet[match(colnames(af), barcode_info$barcode)]
...
saveRDS(af1,file.path(write.path,"sc3cl_splici.rds"))
...
saveRDS(drop_empty,file.path(write.path,"sc3cl_drop_empty.rds"))

drop_sc1<- SingleCellExperiment(
  assays = list(counts = as.matrix(drop_mat[,defaultDrops(drop_mat)]))
)
...
drop_sc1$doublet <- barcode_info$doublet[match(colnames(drop_sc1), barcode_info$barcode)]
...
saveRDS(drop_sc11,file.path(write.path,"sc3cl_drop.rds"))


#cellranger
cellr_path <-"/stornext/HPCScratch/home/you.y/preprocess_update/raw_results/cellranger/sc_3cl/combined/outs/raw_feature_bc_matrix/"
...
saveRDS(cellr_empty,file.path(write.path,"sc3cl_cellranger_empty.rds"))

#cellrangr_sc <- cellrangr_sc1[,which(cellr_empty$FDR<0.001)]
...
cellrangr_sc$doublet <- barcode_info$doublet[match(colnames(cellrangr_sc), barcode_info$barcode)]
...
saveRDS(cellrangr_sc1,file.path(write.path,"sc3cl_cellranger.rds"))
