...
saveRDS(zero.d,"SCE/raw/zeros/mus2_zero.rds")

pdf("results/raw/mus2/zero_all.pdf",width = 5,height = 4)
...
pdf("results/raw/mus2/zero.pdf",width = 10,height = 4)
...
saveRDS(gene_biotype,"SCE/raw/biotypes/mus2_gene_biotype.rds")
...
pdf("results/raw/mus2/biotype_count.pdf",width =6, height = 4)
...
pdf("results/raw/mus2/upsetr.pdf",width = 5,height = 5)
...
pdf("results/raw/mus2/genes_violin.pdf",width = 5,height = 3.5)
...
pdf("results/raw/mus2/total_counts_overlap.pdf",width = 5,height = 3.5)
...
pdf("results/raw/mus2/genes_overlap.pdf",width = 5,height = 3.5)
...
saveRDS(dgenes,"SCE/raw/overlap/mus2_dgenes.rds")
saveRDS(umi_counts,"SCE/raw/overlap/mus2_umi_counts.rds")
...
pdf("results/raw/mus2/correlation.pdf",width = 5,height = 3.5)
...
saveRDS(cors,"SCE/raw/cor/mus2_cors.rds")
...
saveRDS(filter.d,"SCE/raw/filter/mus2_filter.rds")

library(ggsci)

#mito

pdf("results/raw/mus2/mito_filter.pdf")
...
#total
pdf("results/raw/mus2/total_filter.pdf",width = 6,height = 4)
...
ggplot(data=filter.d,aes(x=preprocess,y=total_counts_per_cell)) +
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5),
        text=element_text(size=15)) +
  geom_text(data=filter.d %>% group_by(preprocess,kept) %>% summarise(n=n()) ,aes(x=preprocess,y=1.5,label=n),size=3) +
  #scale_fill_manual(values = mycol)+
  theme_bw() + labs(x="",y="Total counts per cell",fill="preprocess")
dev.off()


pdf("results/raw/mus2/total_filter_density.pdf",width = 6,height = 4)
...
#detected
pdf("results/raw/mus2/detected_filter.pdf",width = 6,height = 4)
...
pdf("results/raw/mus2/kept_label_detected.pdf",width = 6,height = 4)
ggplot(data=kept.f,aes(x=preprocess,y=detected)) +
  ...
  geom_text(data=kf ,aes(x=preprocess,y=80,label=count),size=3) +
  coord_flip()+
  scale_y_log10() +scale_color_jco()+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5),
        text=element_text(size=15)) +
  ...
  theme_bw() + labs(x="",y="Number of detected genes per cell",fill="preprocess")
dev.off()
pdf("results/raw/mus2/kept_label_total.pdf",width = 6,height = 4)
...
number_cells <- data.frame(preprocess=c("scPipe","Cell Ranger", "kallisto bustools","salmon alevin",
                                        "Optimus","dropSeqPipe","zUMIs"),
                           single3=c(713,683,1085,683,709,732,1042),
                           single5=c(1743,1092,968,1094,2581,1336,1657),
                           tissue2=c(3260,4666,4437,7639,3915,4045,5084))
pdf("./results/raw/cell_num_single3.pdf",width=5,height = 2.5)
ggplot(number_cells) + geom_col(aes(x=preprocess,y=single3,fill=preprocess)) +
  scale_fill_manual(values=mycol) + coord_flip() + theme_bw() +labs(x="")
dev.off()
pdf("./results/raw/cell_num_single5.pdf",width=5,height = 2.5)
ggplot(number_cells) + geom_col(aes(x=preprocess,y=single5,fill=preprocess)) +
  scale_fill_manual(values=mycol) + coord_flip() + theme_bw() +labs(x="")
dev.off()
