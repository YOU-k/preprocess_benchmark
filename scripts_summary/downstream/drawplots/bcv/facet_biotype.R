...
dir(SCE_drop.path)
#"mus1","mus2",
designs=c("mus2","sc3cl","sc5clv3","sc5cl","pbmc5k","pbmc10k")
designs=c("sc5clv3")
for (d in designs){
  design=d
  if (d %in% c("mus1","mus2")){
  ...
  for (i in 1:nrow(datasets)){
    ...
    saveRDS(disp,paste0("/stornext/HPCScratch/home/you.y/preprocess_update/SCEs/disp/",design,"_",datasets$data[i],"_disp_pro.rds"))
  ...
  saveRDS(bcv_pro,paste0("/stornext/HPCScratch/home/you.y/preprocess_update/SCEs/bcv/",design,"_bcv_pro.rds"))
  ...
  for (i in 1:nrow(datasets)){
    ...
    dd <- model.matrix(~celltype)
    estimateDisp(counts(tmp_sce),design=dd,robust = TRUE) -> disp
    saveRDS(disp,paste0("/stornext/HPCScratch/home/you.y/preprocess_update/SCEs/disp/",design,"_",datasets$data[i],"_disp_lnc.rds"))
    tmp_bcv <- data.frame(method=datasets$data[i],
                          x=log2(rowMeans(normcounts(tmp_sce))),
                          y=disp$trended.dispersion)
  ...
  saveRDS(bcv_lnc,paste0("/stornext/HPCScratch/home/you.y/preprocess_update/SCEs/bcv/",design,"_bcv_lnc.rds"))
  ...
  for (i in 1:nrow(datasets)){
    ...
    if (nrow(tmp_sce)>100){
      ...
      dd <- model.matrix(~celltype)
      estimateDisp(counts(tmp_sce),design=dd,robust = TRUE) -> disp
      saveRDS(disp,paste0("/stornext/HPCScratch/home/you.y/preprocess_update/SCEs/disp/",design,"_",datasets$data[i],"_disp_pseudo.rds"))
      tmp_bcv <- data.frame(method=datasets$data[i],
                            x=log2(rowMeans(normcounts(tmp_sce))),
                            y=disp$trended.dispersion)
    }else{tmp_bcv<- tibble()}
  ...
  saveRDS(bcv_pseudo,paste0("/stornext/HPCScratch/home/you.y/preprocess_update/SCEs/bcv/",design,"_bcv_pseudo.rds"))
  
  pdf(paste0(write.path,d,"_bcv.pdf"),width = 9,height = 3.5)
