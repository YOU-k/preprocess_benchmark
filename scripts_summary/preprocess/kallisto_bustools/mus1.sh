...
kallisto bus -i $work_p/mus.idx -o mouse1/ -x 10xv2 -t 4 $seq_p/bamtofastq_S1_L002_R1.fastq.gz $seq_p/bamtofastq_S1_L002_R2.fastq.gz

#run bustools
cd mouse1/
mkdir genecount/ tmp/
$bustools correct -w $data_p/10xv2_whitelist.txt -p output.bus | $bustools sort -t 4 -T ./tmp -p - | $bustools count -o genecount/genes -g $ref_p/test.tsv -e matrix.ec -t transcripts.txt --genecounts - 
