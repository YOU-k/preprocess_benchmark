...
START=$(date +%s)
#build an index
$salmon alevin --dumpMtx -l ISR -1 $seq_p/bamtofastq_S1_L002_R1.fastq.gz -2 $seq_p/bamtofastq_S1_L002_R2.fastq.gz --chromium -i $work_p/mus2.idx -p 10 -o $work_p/mus1/alevin_output --tgMap $ref_p/test12.tsv --keepCBFraction 1
END=$(date +%s)
DIFF=$(( $END - $START ))
